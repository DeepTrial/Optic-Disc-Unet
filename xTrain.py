...
def SampleTest(gen,batch_size,patch_h,patch_w):
    (X,Y)=next(gen)
    visualize_all(group_images(X[:, 0:1, :, :],5), './DataSet/patchSample.jpg')
    Y=np.reshape(Y,[batch_size,patch_h,patch_w,num_lesion_class+1])
    Y=Y.transpose(0,3,1,2)
    visualize_all(group_images(Y[:,0:1,:,:],5), './DataSet/groundtruthSample.jpg')
...
if NewTrain:
    model = odModel.AttnUnet(1, patch_height, patch_width)
    model.compile(optimizer='adam', loss="categorical_crossentropy", metrics=[odModel.fbeta_score, 'accuracy'])#['categorical_accuracy'])
    ...
    model = odModel.get_unet(1, patch_height, patch_width)
    model.load_weights('./DataSet/'+name_experiment + '_'+best_last+'_weights.h5')
    model.compile(optimizer='sgd', loss="categorical_crossentropy", metrics=[odModel.dice_coef])
    ...
    model.load_weights('./DataSet/'+name_experiment + '_'+best_last+'_weights.h5')
    model.compile(optimizer='sgd', loss="categorical_crossentropy", metrics=[odModel.dice_coef])#'categorical_accuracy'])
